...
app.post('/api/reports/upload', authMiddleware, upload.single('file'), (req, res) => {
  const db = req.db;
  ...
  const report = {
    ...
    file: req.file ? { url: '/uploads/' + req.file.filename, originalname: req.file.originalname } : null
  };
  db.reports.push(report);
  saveDB(db);

  // notificar destinatários conectados via SSE
